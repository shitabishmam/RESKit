...
def test_rooftop_pv_era5_unvalidated(pt_pv_placements):
    gen = rooftop_pv_era5_unvalidated(
    ...
    assert np.isclose(float(gen["capacity_factor"].fillna(0).mean()), 0.05194219175264894)
    assert np.isclose(float(gen["total_system_generation"].fillna(0).mean()), 103.88438350529788)
...
def test_openfield_pv_era5_unvalidated(pt_pv_placements):
    ...
    assert np.isclose(float(gen["elev"].fillna(0).mean()), 300.0)
    assert np.isclose(float(gen["global_horizontal_irradiance"].fillna(0).mean()), 32.51543182317497)
    assert np.isclose(float(gen["direct_horizontal_irradiance"].fillna(0).mean()), 15.41170374927935)
    assert np.isclose(float(gen["surface_wind_speed"].fillna(0).mean()), 1.696705522977915)
    assert np.isclose(float(gen["surface_pressure"].fillna(0).mean()), 38651.440408516704)
    assert np.isclose(float(gen["surface_air_temperature"].fillna(0).mean()), 0.9659668062938231)
    ...
    assert np.isclose(float(gen["module_dc_voltage_at_mpp"].fillna(0).mean()), 12.806209106252805, rtol=0.0001)
    assert np.isclose(float(gen["capacity_factor"].fillna(0).mean()), 0.10725306315770565, rtol=0.0001)
    assert np.isclose(float(gen["total_system_generation"].fillna(0).mean()), 214.50612631541125, rtol=0.0001)
...
def test_openfield_pv_merra_ryberg2019(pt_pv_placements):
    ...
    assert np.isclose(float(gen["module_dc_voltage_at_mpp"].fillna(0).mean()), 14.079408128765307, rtol=0.0001)
    assert np.isclose(float(gen["capacity_factor"].fillna(0).mean()), 0.04249361100670641, rtol=0.0001)
    assert np.isclose(float(gen["total_system_generation"].fillna(0).mean()), 84.9872220134128, rtol=0.0001)
